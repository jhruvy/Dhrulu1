...
def test_can_add_user(in_memory_repo):
    new_username = 'djoh991'
    new_password = 'YeeeYaww12345'
...
def test_authentication_with_invalid_credentials(in_memory_repo):
    ...
    with pytest.raises(auth_services.AuthenticationException):
        auth_services.authenticate_user(new_username, '2579643678', in_memory_repo)
...
def test_can_get_movie(in_memory_repo):
    ...
    assert movie_as_dict['actors'] == [Actor("Matt Damon"), Actor("Tian Jing"),
                            Actor("Willem Dafoe"), Actor("Andy Lau")]
...
def test_get_reviews_for_movie(in_memory_repo):
    movie_id = 1
    review_text = "Coool film!"
    new_username = 'djoh991'
    new_password = 'happy12345'
    auth_services.add_user(new_username, new_password, in_memory_repo)
    movie_library_services.add_review(movie_id, review_text, new_username, in_memory_repo)
